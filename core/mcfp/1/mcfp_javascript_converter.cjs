...
const mjc = function(mfb) {
    ...
    this.convert_effect = function(target_attribute, formula_block, effect_type, target_attribute_property = 'value') {
        let return_data = {function: null, dependencies: new Set(), target_attribute: target_attribute, target_property: target_attribute_property, use_dices: false};
        let [converted_formula, attribute_dependencies, use_dices] = mjc_this.convert_formula(formula_block);
        if (converted_formula !== null) {
            return_data.function = new Function('attributes', 'functions', 'self_attribute', ...(use_dices ? ['dices_iterator'] : []),
                effect_type === '.='
            ...
            return_data.use_dices = use_dices;
        }
        return return_data;
    }
        ...
        switch (formula_block.constructor) {
            ...
            case mfb.Function:
                ...
                for (let arg of formula_block.args) {
                    let [converted_argument, argument_attribute_dependencies, argument_uses_dices] = this.convert_formula(arg);
            ...
            case mfb.Array:
                ...
                for (let element of formula_block.array) {
                    let [converted_argument, argument_attribute_dependencies, argument_uses_dices] = this.convert_formula(element);
                    if (converted_argument === null) return [null, []];
                    for (let dependency of argument_attribute_dependencies) attribute_dependencies.add(dependency);
                    use_dices ||= argument_uses_dices;
                ...
                break;
            case mfb.Dice:
                let [amount, amount_attribute_dependencies, amount_uses_dices] = this.convert_formula(formula_block.amount);
                let [magnitude, magnitude_attribute_dependencies, magnitude_uses_dices] = this.convert_formula(formula_block.magnitude);
                ...
                use_dices = true;
                // ^ amount_uses_dices & magnitude_uses_dices aren't important, because dice is already used here
                converted_formula = `Array(${amount}).fill(undefined).map((value) => (dices_iterator.get(${magnitude}))).reduce((partialSum, a) => partialSum + a, 0)`;
                break;
            default:
                return [null, []];
        }
        return [converted_formula, attribute_dependencies, use_dices];
    }
}
